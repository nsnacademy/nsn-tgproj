...
export function Home({ onNavigate, refreshKey }: HomeProps) {
  ...
  // Эффект фокуса при скролле (основная механика)
  useEffect(() => {
    const updateCardStyles = (): void => {
      ...
      cards.forEach((card: Element) => {
        ...
        // Scale effect - плавное масштабирование
        const scale = 1 - ratio * 0.05;
        cardElement.style.transform = `scale(${scale})`;
        
        // Opacity effect - плавное изменение прозрачности
        const opacity = 1 - ratio * 0.3;
        cardElement.style.opacity = opacity.toString();
        
        // Border effect for focused card - выделение центральной карточки
        if (ratio < 0.1) {
  ...
  // Демо-данные для красивого отображения (можно заменить реальными)
