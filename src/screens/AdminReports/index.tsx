...
export default function AdminReports({ challengeId, onBack }: Props) {
  ...
  /* === LOAD CHALLENGE === */
  useEffect(() => {
    console.log('📋 [ADMIN REPORTS] Загрузка данных вызова:', challengeId);
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error) {
          console.error('❌ [ADMIN REPORTS] Ошибка загрузки вызова:', error);
        } else {
          console.log('✅ [ADMIN REPORTS] Данные вызова загружены:', data);
          setChallenge(data);
        }
      });
  }, [challengeId]);
  ...
  useEffect(() => {
    if (!challenge) {
      console.log('⏳ [ADMIN REPORTS] Ожидание загрузки challenge...');
      return;
    }
    ...
    console.log('🔍 [ADMIN REPORTS] Загрузка отчетов:', {
      challengeId,
      dayIndex,
      reportDate,
    ...
    supabase
      ...
      .then(async ({ data, error }) => {
        if (error) {
          console.error('❌ [ADMIN REPORTS] Ошибка загрузки отчетов:', error);
          return;
        }

        console.log('✅ [ADMIN REPORTS] Отчеты загружены:', {
          count: data?.length || 0,
          reports: data?.map(r => ({
            id: r.id,
        ...
        if (!data || data.length === 0) {
          console.log('ℹ️ [ADMIN REPORTS] Нет отчетов за этот день');
          return;
        }
        ...
        console.log('📸 [ADMIN REPORTS] Медиа файлы для загрузки:', {
        ...
        for (const path of allMediaPaths) {
          if (mediaUrls[path]) {
            console.log('♻️ [ADMIN REPORTS] Медиа уже загружено, используем кэш:', path);
            continue;
          }

          setLoadingMedia(prev => ({ ...prev, [path]: true }));
          console.log('⏳ [ADMIN REPORTS] Запрос signed URL для:', path);
          ...
          if (signedError) {
            console.error('❌ [ADMIN REPORTS] Ошибка получения signed URL:', {
              path,
              error: signedError
            });
          } else if (signed?.signedUrl) {
            console.log('✅ [ADMIN REPORTS] Signed URL получен:', {
              path,
        ...
        if (Object.keys(urls).length > 0) {
          console.log('📦 [ADMIN REPORTS] Обновление mediaUrls:', {
  ...
  if (!challenge) {
    console.log('⏳ [ADMIN REPORTS] Рендер заглушки загрузки...');
    return (
      <SafeArea>
        <FixedTop>
          <Header>
  ...
  console.log('📊 [ADMIN REPORTS] Статистика отчетов:', {
    day: dayIndex + 1,
    stats,
    activeTab
  });
  ...
  console.log('🎨 [ADMIN REPORTS] Отчеты для отображения:', {
    tab: activeTab,
  ...
  const updateStatus = async (
    ...
    console.log('🔄 [ADMIN REPORTS] Обновление статуса отчета:', {
      reportId,
      status,
      rejectionReason
    });
    ...
    if (error) {
      console.error('❌ [ADMIN REPORTS] Ошибка обновления статуса:', error);
      return;
    }

    console.log('✅ [ADMIN REPORTS] Статус обновлен:', data);
  ...
  const handleBackToAdmin = () => {
    console.log('👈 [ADMIN REPORTS] Возврат к списку вызовов');
    onBack(); // onBack приходит из App.tsx и ведет на 'admin'
  };

  const openFullscreen = (url: string) => {
    console.log('🖼️ [ADMIN REPORTS] Открытие полноэкранного режима:', url.substring(0, 50) + '...');
      ...
      <FixedTop>
        ...
        <DaySwitcher>
          <NavButton
            disabled={dayIndex === 0}
            onClick={() => {
              console.log('⬅️ [ADMIN REPORTS] Переключение на предыдущий день:', dayIndex);
              setDayIndex(d => d - 1);
            }}
          >
            ←
          ...
          <NavButton
            ...
            onClick={() => {
              console.log('➡️ [ADMIN REPORTS] Переключение на следующий день:', dayIndex + 2);
              setDayIndex(d => d + 1);
            }}
          >
            →
                    ...
                    <>
                      ...
                      <MediaGrid>
                        {r.proof_media_urls.map((path, i) => {
                          ...
                          console.log(`🖼️ [ADMIN REPORTS] Отображение медиа ${i + 1}:`, {
                            path,
                            hasUrl: !!url,
                            isLoading,
                            status: r.status
                  ...
                  <>
                    {rejectingReportId !== r.id ? (
                      <Actions>
                        ...
                        <RejectButton
                          onClick={() => {
                            console.log('🔴 [ADMIN REPORTS] Начало отклонения отчета:', r.id);
